...
def fill_hdf5_from_artist(h5,artist):
    ...
    group.similar_artists.append( np.array([x.id for x in artist.get_similar(results=100)],dtype='S20') )
    metadata.cols.idx_artist_terms[0] = 0
    group.artist_terms.append( np.array([x.name for x in artist.get_terms()],dtype='S256') )
...
def fill_hdf5_from_song(h5,song):
    ...
    metadata.cols.artist_location[0] = song.get_artist_location().location.encode('utf-8') if song.get_artist_location().location else b''
    metadata.cols.artist_longitude[0] = song.get_artist_location().longitude
    metadata.cols.artist_name[0] = song.artist_name.encode('utf-8') if song.artist_name else b''
    metadata.cols.song_id[0] = song.id
    metadata.cols.song_hotttnesss[0] = song.get_song_hotttnesss()
    metadata.cols.title[0] = song.title.encode('utf-8') if song.title else b''
    metadata.flush()
...
def fill_hdf5_from_track(h5,track):
    ...
    metadata.cols.artist_name[0] = getattr(track, 'artist', u'').encode('utf-8') if getattr(track, 'artist', u'') else b''
    metadata.cols.release[0] = getattr(track, 'release', u'').encode('utf-8') if getattr(track, 'release', u'') else b''
    metadata.cols.title[0] = getattr(track, 'title', u'').encode('utf-8') if getattr(track, 'title', u'') else b''
